...
def detect_cars(video_file):
    ...
    Conf_threshold = 0.4 # Lowered to catch more detections
    # Non-Maximum Suppression threshold:Same car detected multiple times prevents
    NMS_threshold = 0.4  # Lowered for better detection -removes overlapping boxes

    # Define colors for different classes--size =6
    ...
    while True:
        ...
        try:
            ...
            # print("DETECTION:", classes, scores, boxes) classid arr,confi,boxes
        ...
        for (classid, score, box) in zip(classes, scores, boxes):
            cls=int(classid)
            if class_name[cls] == "car":  # assuming 'car' is the class name for cars in your classes.txt
                car_count += 1
                color = COLORS[int(cls) % len(COLORS)]
                label = f"{class_name[cls]} : {score:.2f}"
        ...
        # Remove counts that are older than 30 seconds--thats why deque users will give lenghy videos assuming - take only last 30 sec
        ...
        # Find peaks in the car count values-local maximas
